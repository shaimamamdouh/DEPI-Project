...
class _ListeningBookViewState extends State<ListeningBookView> {
  ...
  void _fastForward() {
    ...
    setState(() {
      _currentWordIndex = (_currentWordIndex + wordsToSkip).clamp(0, words.length - 1);
      _progress = _currentWordIndex / words.length;
      if (_isPlaying) {
        TextToSpeechHelper.speak(widget.book!.description!, startWordIndex: _currentWordIndex);
      }
    });
  }
  ...
  void _startProgressTimer() {
    ...
    _progressTimer = Timer.periodic(Duration(milliseconds: (updateInterval * 1000).toInt()), (timer) {
      if (!_isPlaying) {
        timer.cancel();
        return;
      }
      setState(() {
        ...
        _currentWordIndex = (_progress * totalWords).round().clamp(0, words.length - 1);
      });
    });
  }
          ...
          children: [
            ...
            Slider(
              ...
              onChanged: (value) {
                ...
                setState(() {
                  _progress = value;
                  _currentWordIndex = (value * words.length).round().clamp(0, words.length - 1);
                });
                if (_isPlaying) {
