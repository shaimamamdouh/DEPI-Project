    ...
    : super(
        BookChatBotState(
          messages: [
            ChatMessage(
              text:
                  'Hello! I\'m the Book Chat Bot. I can suggest books based on category, author, or search for a specific book. What would you like?',
              isBot: true,
            ),
          ],
        ),
      ...
      if (userInput.toLowerCase().contains('category')) {
        ...
        final response =
            books.isEmpty
                ...
                : 'Here are 3 books in the "$category" category:\n${books.asMap().entries.map((entry) => '${entry.key + 1}. "${entry.value.title}" by ${entry.value.author ?? 'Unknown'}\n   - Description: ${entry.value.description?.substring(0, entry.value.description!.length > 100 ? 100 : entry.value.description!.length) ?? 'No description'}...').join('\n')}';
        emit(
          BookChatBotState(
            messages: [
              ...state.messages,
        ...
        final response =
            books.isEmpty
                ...
                : 'Here are 3 books by "$author":\n${books.asMap().entries.map((entry) => '${entry.key + 1}. "${entry.value.title}"\n   - Description: ${entry.value.description?.substring(0, entry.value.description!.length > 100 ? 100 : entry.value.description!.length) ?? 'No description'}...').join('\n')}';
        emit(
          BookChatBotState(
            messages: [
              ...state.messages,
